...
<tr>
  ...
  <td>Used by microG as an alternative to Google's location services. May be disabled in the microG settings.</td>
</tr>
<tr>
...
<tr>
  ...
  <td>SUPL is a service that allows the device to much more quickly get a GPS lock. Currently, the request for GPS satellite information that CalyxOS makes to Google servers includes the device's IMEI (over TLS). We are looking at alternatives to Google's SUPL service.</td>
</tr>
<tr>
  <td>remoteprovisioning.googleapis.com</td>
  <td>Used for "Key Attestation" which proves to applications that the device is genuine. See below for more details.</td>
</tr>
</table>
...
When a device is manufactured, internally it generates a secret that is very difficult to extract from the hardware. A "public" portion of this secret is given to Google by the device manufacturer. When the device is later run, microG will submit proof to Google that the device has the secret without revealing what it is, and receives back from Google a certificate that can be used for different purposes to attest that the device is valid. This system is used for things like SafetyNet, Identity Credential, and Digital Car Key.
