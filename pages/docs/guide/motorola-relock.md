...
To fix this, you can download a newer CalyxOS factory image for your device with the updated ARB index and run the CalyxOS device-flasher or web installer again. Once the image passes the ARB index check, the device-flasher or web installer will relock the bootloader for you to use CalyxOS with maximum security. We always keep the latest CalyxOS images available in [our Installation hub](https://calyxos.org/install/). If you are not able to find the latest image for your device or are not sure if it has the correct ARB index, contact us at [[community]] for more help.
