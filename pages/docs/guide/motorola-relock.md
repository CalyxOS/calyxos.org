...
For example, the previous operating system might have used rollback index 27 while this CalyxOS operating system uses 25. If your phone bootloader was locked in this example situtation, with CalyxOS on a lower index, your phone might refuse to start.

To avoid leaving you in this situation, the device-flasher has left the bootlader unlocked. Do not try to lock it manually while this warning is displayed. Doing so could prevent the phone from turning on and make recovery difficult or impossible.
...
To fix this, download a newer CalyxOS image for your device with an updated ARB index and run the CalyxOS device-flasher again. The device-flasher will relock once it confirms that it is safe to do so.
