...
2. Go to <https://review.calyxos.org/settings/#Profile> - set a Full Name by filling out this field, and note down your username (it will be the same as your GitHub username)
3. Scroll down to <https://review.calyxos.org/settings/#EmailAddresses> - and add the email address you usually use with git. You will receive an email with a link for verification.
...
* Code-Review is used to review the code. +1/-1 are like thumbs up / down. +2 is needed to merge a change, and -2 prevents a change from being merged.
    ...
    * Setting Works +1 and Code-Review +1 on your own changes that you want reviewed / merged is a good practice.
* Once a change has at least one Works +1 and Code-Review +2, a "Submit" button will appear, and you can submit the change by clicking it.
* A change can also be abandoned if it is no longer needed.
* Bypass-Verification is used to override the Works vote in cases where we want to merge changes without having to verify if they are working, eg. simple text changes or typo fixes.
