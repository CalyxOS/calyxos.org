...
### UI
The user interface of the Datura Firewall makes it easier to customize settings. You can use the global toggle switch to set your preference for the framework to process. It also enables access customization for individual apps. If you switch the toggles to restrict a particular access for a specific app, it will pass the app UID down to the framework for processing along with the desired rule.

It appears in CalyxOS as a standalone app called Firewall, which is internally referred to as Datura. It is part of the [external/calyx/datura](https://gitlab.com/CalyxOS/platform_external_calyx_datura) repo. Datura Firewall is built separately in production via gradle, after which the production build is uploaded to the [prebuilts/calyx/datura](https://gitlab.com/CalyxOS/platform_prebuilts_calyx_datura).
...
<<<<<<< PATCH SET (cb0ee6c43cba54bf4a67bb0217cf0c2c5b970e6d Update firewall documentation)
...
>>>>>>> BASE      (ff85ebc24c5ce125f4ea0d927af948e027b79aba pages: Update included apps)
