...
<strong>[[Install => install]]</strong> / <strong>[[{{name}} => {{codename}}]]</strong> / <strong><a href="#">{{os_name}}</a></strong>
...
\{% include install/{{os_codename}}_drivers.md \%}

\{% include install/{{os_codename}}_flasher.md \%}

### Download firmware image

<a class="btn" href="{{factory_link}}">Download CalyxOS Image</a>

Save this image in the same directory as device-flasher. This image will **only** work for **{{name}} ({{codename}})**. Any attempt to install this image on a different device model my destroy the Android device.
