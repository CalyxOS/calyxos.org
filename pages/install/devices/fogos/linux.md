...
<a class="btn" href="https://release.calyxinstitute.org/fogos-factory-25607220.zip">Download CalyxOS Image</a>


Save this image in the same directory as device-flasher. This image will **only** work for **moto g34 5G (fogos)**. Any attempt to install this image on a different device model may brick (destroy) the Android device.
...
{% assign image = "https://release.calyxinstitute.org/fogos-factory-25607220.zip" | split: "/" | last %}
{% include install/linux_image_digest.md filename=image %}

And ensure the result says `6941a1a4865590f032666bb66add457a7bd6ee086fc36b86feb8ec49df53f1d1`. This is the unique digest of the image file. If it doesn't match, then you have a corrupted or incorrect file.

#### Verify Signature

For the highest level of confidence, you can optionally <a href="https://release.calyxinstitute.org/fogos-factory-25607220.zip.minisig">download the signature file</a> and then [[verify image signature => verify]].
...
{% if site.data.install.fogos.unlock_notes %}
{% include {{site.data.install.fogos.unlock_notes}} %}
{% endif %}
